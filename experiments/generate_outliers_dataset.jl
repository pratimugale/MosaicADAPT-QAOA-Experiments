...
function main()
    ...
    if !isfile(outliers_list_path)
        println("Error: Outliers file list not found at $outliers_list_path")
        return
    end
    ...
    # OUTLIER RUNNER ONLY USES THE TAILORED TRIPLE POOL
    strategies = [
        (false, "tailored_triple_pool", 0.0, "Greedy_TailoredPool"),
        (true, "tailored_triple_pool", 0.0, "KaMIS_TailoredPool")
    ]
    ...
    @multibreak for (local_idx, cnf_filename) in enumerate(my_files)
        ...
        # Re-derive the ID from the filename since we aren't sweeping sequentially
        # Assuming filename format is something like test_balanced_sat_X_vars_Z.cnf
        m = match(r"sat_\d+_vars_(\d+)\.cnf", cnf_filename)
        global_idx = m !== nothing ? parse(Int, m.captures[1]) : local_idx

        println("Processing Outlier: $cnf_filename (Derived Global ID: $global_idx)")
