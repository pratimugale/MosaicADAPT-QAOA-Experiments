...
import .MIS_TETRIS_ADAPT: TetrisConfig, BenchmarkResult, TetrisResult
import .MIS_TETRIS_ADAPT: run_layerstopped_tetris, parse_cnf_file
import ADAPT
...
function run_comprehensive_benchmark()
    ...
    for n_vars in qubit_counts
        ...
        for type in ["balanced", "triangle"]
            ...
            # Offset seed by type to avoid identical seeds if relevant (though N offset helps)
            type_offset = (type == "triangle") ? 5000 : 0
            current_seed = base_seed + (n_vars * 1000) + type_offset
        ...
        for (idx, cnf_filename) in enumerate(target_files)
            ...
            if captured_config_base === nothing
                captured_config_base = Dict(
                    ...
                    "layer_limit" => base_config.layer_stopper_max,
                    "configurations" => [
            ...
            configs_to_run = [
                ...
                (false, "qaoa_double_pool", 0.0, "Greedy_DoublePool"),
                (true, "qaoa_double_pool", 0.0, "KaMIS_DoublePool"),
            ]

            for (use_kamis, pool_type, pct_tail, label) in configs_to_run
                t_res = run_layerstopped_tetris(
                ...
                println("  > $label: Energy=$(t_res.final_energy), Layers=$(t_res.num_adapt_layers), 1st_Layer_Grad_Sum=$(t_res.first_layer_gradient_sum)")
            end
    ...
    println("\n=== GREEDY vs KAMIS Benchmark Complete ===")
