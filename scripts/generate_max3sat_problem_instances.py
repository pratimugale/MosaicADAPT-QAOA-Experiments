...
def get_canonical_string_and_hash(cnf_path):
    ...
    unique_clauses = set(tuple(c) for c in sorted_clauses)
    has_duplicates = len(unique_clauses) < len(sorted_clauses)
    
    return canonical_str, h, has_duplicates
    ...
    for i in range(num_instances):
        ...
        while True:
            ...
            try:
                ...
                canonical_str, h, has_duplicates = get_canonical_string_and_hash(path)
                
                if has_duplicates:
                    print(f"WARNING: Duplicate clauses detected in instance i={i}. Regenerating...")
                ...
                if registry.is_duplicate(h):
                    print(f"WARNING: Duplicate instance detected for i={i} (Hash Collision: {h[:8]}...). Regenerating...")
                    if os.path.exists(path):
                        os.remove(path)
                    seed_offset += 1000
                    continue
